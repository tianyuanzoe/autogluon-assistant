...
infer_test_id_column_template = "The Id column for the sample output is {output_id_column} but a column with that name is not found in the test data. Which column from the following list of test data columns is most likely to be the Id column:\n{test_columns}\nIf no reasonable Id column is present, for example if all the columns appear to be similarly named feature columns, response with the value NO_ID_COLUMN_IDENTIFIED"
