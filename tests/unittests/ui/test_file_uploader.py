...
from autogluon.assistant.ui.file_uploader import save_description_file, description_file_uploader, file_uploader, get_user_data_dir
...
def test_file_uploader_csv(mock_uploader, mock_session_state):
    ...
    with patch('pandas.read_csv', return_value=df) as mock_read_csv:
        ...
        pd.testing.assert_frame_equal(
            mock_session_state.uploaded_files['test.csv']['df'],
            df
        )
...
def test_file_uploader_excel(mock_uploader, mock_session_state):
    ...
    with patch('pandas.read_excel', return_value=df) as mock_read_excel:
        ...
        pd.testing.assert_frame_equal(
            mock_session_state.uploaded_files['test.xlsx']['df'],
            df
        )
